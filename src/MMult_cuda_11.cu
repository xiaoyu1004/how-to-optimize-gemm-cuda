 ...
 * 3. 写回 C 矩阵时，引入 st128 方式，直接写入 global 地址，有效果。目前卡在writeback 方式上
                                                          ...
                                                          float *c)
{
    ...
    uint32_t a_sts_addr = smem_u32addr(ashare + (threadIdx.x % 8) * SMEM_LDA + (threadIdx.x / 8) * 4);
    uint32_t b_sts_addr = smem_u32addr(bshare + (threadIdx.x / 32) * SMEM_LDB + (threadIdx.x % 32));
...
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
            ldg32_nc_0(b_ldg_reg[i], (const char *)(b + from_b) + i * 32 * sizeof(float));
        }
#pragma unroll
        for (int i = 0; i < 4; ++i)
        {
...
#pragma unroll
        ...
        {
            lds128(panelA[0], panelA[1], panelA[2], panelA[3], aptr_base + (subk * SMEM_LDA) * sizeof(float));
            lds128(panelA[4], panelA[5], panelA[6], panelA[7], aptr_base + (subk * SMEM_LDA + 64) * sizeof(float));

            lds128(panelB[0], panelB[1], panelB[2], panelB[3], bptr_base + (subk * SMEM_LDB) * sizeof(float));
            lds128(panelB[4], panelB[5], panelB[6], panelB[7], bptr_base + (subk * SMEM_LDB + 64) * sizeof(float));
...
#if 1
    uint32_t c_sts_addr = smem_u32addr(reinterpret_cast<float *>(smem) + ((threadIdx.x / 16) * 4 * SMEM_LDC) + (threadIdx.x % 16) * 4);

    // 8x32
    float *C_lds_ptr = (float *)(smem) + (threadIdx.x / 32 * 8) * SMEM_LDC + (threadIdx.x % 32);
