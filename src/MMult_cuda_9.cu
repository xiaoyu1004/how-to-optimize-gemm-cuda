 ...
 * 个数据，放到 smem 里是 1x4，32x8 个线程加载 128x8 大小的 subA，变成  8x128
 *
 * matrixB 不 transpose，就是单纯的加载。32 thread 合并访问。 thread i 访问 [i, i+32, i+64, i+96]
 *
 */
                                                          ...
                                                          float *c)
{
    ...
    float *ashare = reinterpret_cast<float *>(smem); // 16k shared mem for A
